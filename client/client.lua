...
RegisterNUICallback('BuyWagon', function(data, cb)
    ...
    -- [FIX] เช็คว่ามีชื่อส่งมาจาก UI หรือไม่ ถ้ามีให้บันทึกเลย ไม่ต้องเปิดคีย์บอร์ดซ้ำ
    if data.name and data.name ~= "" then
        local wagonInfo = {wagonData = data, name = data.name}
        local wagonSaved = Core.Callback.TriggerAwait('bcc-wagons:SaveNewWagon', wagonInfo)
        if wagonSaved then
            -- ซื้อสำเร็จ ปิดเมนูหรือรีเฟรชตามต้องการ
            WagonMenu()
        end
        return -- จบการทำงานตรงนี้
    end

    -- ถ้าไม่มีชื่อค่อยเปิดคีย์บอร์ด (Logic เดิม)
...
-- [เพิ่ม] Callback สำหรับเรียกเกวียน (SpawnWagon)
RegisterNUICallback('SpawnWagon', function(data, cb)
    cb('ok')
    -- เรียกฟังก์ชัน SpawnWagon เดิมที่มีอยู่แล้ว
    SpawnWagon(data.WagonModel, data.WagonName, true, data.WagonId)
end)

-- [เพิ่ม] Callback สำหรับเก็บเกวียน (StoreWagon)
RegisterNUICallback('StoreWagon', function(data, cb)
    cb('ok')
    -- เรียกฟังก์ชัน ReturnWagon เดิมที่มีอยู่แล้ว
    ReturnWagon()
end)